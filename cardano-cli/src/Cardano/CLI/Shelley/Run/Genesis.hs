...
runGenesisCreateStaked (GenesisDir rootdir)
  ...
  delegations <- liftIO $ Lazy.forM distribution $ computeInsecureDelegation network
...
computeInsecureDelegation ::
  ...
  -> IO Delegation
computeInsecureDelegation nw pool = do
    paymentVK <- fmap getVerificationKey $ generateInsecureSigningKey AsPaymentKey
    stakeVK   <- fmap getVerificationKey $ generateInsecureSigningKey AsStakeKey
